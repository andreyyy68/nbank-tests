...
class TestChangeUsername:
    def test_valid_change_username(self, user_page, api_manager):
        ...
        name = EditProfilePage(user_page).open().edit_profile_name(data_user.name).button_save_change(AlertMessage.NAME_UPDATE).get_profile_name(data_user.name)

        username = api_manager.user_steps.get_changed_username(name)
    ...
    def test_invalid_change_username(self, user_page, api_manager):
        ...
        current_username = api_manager.user_steps.get_profile().name
